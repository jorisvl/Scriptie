...
lon4=np.array([])
for i in range(len(lat)):
    a=(lat5[i]*2*np.pi)/360
    b=(lon5[i]*2*np.pi)/360
...
latcor2=latcor**2
loncor2=loncor**2
tijdsverschil=np.delete(np.insert(tijd,0,tijd[0]),-1)
print latcor[0:9]
...
for i in range(len(latcor)):
    if np.std(latcor[i:i+n]) >stdbslat and i+n-1<len(latcor):
        latcor=np.delete(latcor,i+n-1)
        loncor=np.delete(loncor,i+n-1)
        tijd=np.delete(tijd,i+n)

print len(latcor)

for i in range(len(loncor)):
    if np.std(loncor[i:i+n]) >stdbslon and i+n-1<len(latcor):
        latcor=np.delete(latcor,i+n-1)
        loncor=np.delete(loncor,i+n-1)
        tijd=np.delete(tijd,i+n)
...
for i in range(len(latcor)):
    if np.std(latcor[-i-n:-i]) >stdbslat:
...
for i in range(len(loncor)):
    if np.std(latcor[-i-n:-i]) >stdbslon:
        latcor=np.delete(latcor,-i-n+1)
        loncor=np.delete(loncor,-i-n+1)
        tijd=np.delete(tijd,-i-n+1)
# Hier reken ik de snelheid uit
tijdsverschil=tijd-np.delete(np.insert(tijd,0,0),-1)
u=latcor/tijdsverschil
v=loncor/tijdsverschil
snelheid=np.power((np.power(u,2)+np.power(v,2)),0.5)/3600
#Hier vind ik de tijdwaardes zonder snelheid
tijdnul=np.array([])
for element in range(79201):
    if element not in tijd:
